/*
  ...
  Most graphical “paint” applications, have a ‘paintcan fill’ function that floods part of an image with
  a certain color. We change the image as if we painted a canvas: a two-dimensional array of integers, 
  where each integer represents a color for that pixel. The canvas Array.length is the Y dimension of 
  our canvas; each spot in the canvas array is a row in our image, with a length equal to our canvas’ 
  X dimension. You are given a canvas (2 dimensional array of integers), starting coordinate 
  ...
  adjacent via X or Y to another pixel you will change. Note: diagonally related pixels are not 
  considered adjacent.
  ...
  and startXY of [2,1], and newColor of 1.

  we examine the cells that are directly (not diagonally) adjacent to startXY. If any have a value of 3 
  (the original value at startXY), we change its value to 1 ( newColor ) and repeat the process with its 
  directly-adjacent neighbor cells. We repeat this until th e entire zone of similarly-colored cells is 
  changed .
...
/* 
define a base case. - if the start XY is = new color.
  return the new color.
 
check starting XY adjacents to see if they are the same number/color
  if they are, change to the new color,
  if they are not, skip.
  >>>Recursion loop.
 
how do we pull index of nested index to check all of the values?
  start XY
 
if single array has same values touching each other, change those values to touched value.
  if [1,2,3,3,4] >> all of the 3's would change to the newColor
 
[x-1,y], [x+1,y],[x,y-1],[x,y+1]
 
 
 
then compare to the other arrays. and change their values if matching current value.
